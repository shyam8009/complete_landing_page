...
function generateHotspotPath(baseX: number, baseY: number, radiusX: number, radiusY: number, phaseOffset: number) {
  ...
  for (let i = 0; i < TOTAL_FRAMES; i++) {
    ...
    // Z-depth calculation to hide the hotspot when it rotates *behind* the device
...
export function Interactive360RhinoBlackViewer() {
  ...
  const hotspots = useMemo(() => [
    {
      id: 'omni',
      label: 'Omni-Directional Neutralization',
      icon: <Radio className="w-5 h-5 text-[#84CC16]" />,
      path: generateHotspotPath(50, 25, 25, 8, 0),
    },
    {
      id: 'manpack',
      label: '10 kg Manpack Design',
      icon: <Briefcase className="w-5 h-5 text-[#84CC16]" />,
      path: generateHotspotPath(50, 60, 35, 12, Math.PI / 2),
    },
    {
      id: 'battery',
      label: 'Extended Operational Duration',
      icon: <Battery className="w-5 h-5 text-[#84CC16]" />,
      path: generateHotspotPath(50, 80, 20, 5, Math.PI),
    }
  ], []);
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= TOTAL_FRAMES; i++) {
      ...
      img.src = `/infinity-rhino-black-360/frame_${paddedIndex}.webp`;
