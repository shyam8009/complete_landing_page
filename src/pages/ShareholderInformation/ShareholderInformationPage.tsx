...
function Hero() {
  return (
    <section className="relative w-full min-h-[75vh] md:min-h-[85vh] overflow-hidden bg-black text-white" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {/* Content */}
      <div className="relative z-20 h-full min-h-[75vh] md:min-h-[85vh] flex flex-col justify-center pt-20 sm:pt-28 md:pt-40 px-4 sm:px-8 md:px-16 lg:px-20 pb-16 md:pb-24">
        <motion.div
          initial="hidden"
          animate="visible"
          variants={stagger}
        ...
        >
          ...
          >
            Resources and updates for our valued shareholders and investors.
          </motion.p>
  ...
  return (
    <section className="relative bg-slate-50 py-16 md:py-24 border-b border-slate-200" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      >
        ...
        >
          Our purpose is to present precise, transparent, and insightful subsidiary financial results that demonstrate our dedication to <span className="text-[#84CC16] font-medium">integrity and accountability</span>. By providing clear and detailed financial information, we enable stakeholders to evaluate subsidiary performance effectively and contribute to the overall sustainable growth of our organization.
        </motion.p>
      </motion.div>
    </section>
  );
  ...
  return (
    <section className="relative bg-white py-16 md:py-24" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="relative z-10 max-w-[1100px] mx-auto px-6 sm:px-10">
        ...
        {/* Tabs - styled similarly to the screenshot with pills */}
        <div className="flex flex-wrap gap-2 sm:gap-4 mb-10 pb-4 border-b border-slate-200">
          {TABS.map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 text-xs sm:text-sm font-bold rounded-full transition-all duration-300 border ${
                activeTab === tab 
                  ? 'bg-[#CFA438] text-white border-[#CFA438] shadow-md' 
